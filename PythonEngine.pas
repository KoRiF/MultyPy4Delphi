...
//    PyEval_CallObject: function(callable_obj, args:PPyObject):PPyObject; cdecl;
    ...
    //PyInterpreterState_Get          : function : PPyInterpreterState; cdecl;
    PyInterpreterState_Main          : function : PPyInterpreterState; cdecl;
...
type
  ...
  TWrithingPythonEngine = class(TPythonEngine)
  public
    function Configure(const PythonEnv, PythonVer: String): TWrithingPythonEngine; overload;
    function Configure(const PythonFileDll: String): TWrithingPythonEngine;  overload;
    function Attach(const EnvLib: String = ''): TPythonEngine;
  private
    procedure CollectPyGarbage();
    procedure SoftCleanup();
    procedure OnInitSysPath(Sender: TObject; PathList: PPyObject);
  end;
//-------------------------------------------------------
//--                                                   --
...
//-------------------------------------------------------
  ...
  TPersistentPythonDelphiVar = class (TPythonDelphiVar)
    public
      procedure Init(value: Variant);
      procedure StoreValue();
      procedure RestoreValue();
    private
      _StorageValue: Variant;
  end;
...
begin
  ...
  //PyInterpreterState_Get   := Import('PyInterpreterState_Get');     Py10
  PyInterpreterState_Main  := Import('PyInterpreterState_Main');
...
begin
  ...
  if Initialized and FAutoFinalize then begin
    try
      try
        FFinalizing := True;
        //PyInterpreterState_Delete(_PyInterpreterState);
        Py_Finalize;
      finally
        FFinalizing := False;
        FInitialized := False;
...
begin
  ...
  if Assigned(Py_IsInitialized) then
    ...
    FInitialized := True;
  //_PyInterpreterState := PyInterpreterState_Main();
...
{ TWrithingPythonEngine }

function TWrithingPythonEngine.Attach(const EnvLib: String): TPythonEngine;
begin
  if Self.IsHandleValid then
  begin
    Self.SoftCleanup();//Finalize();
    Self.UnloadDll;
  end;
  ...
  if EnvLib <> '' then
  begin
    Self.DllName := ExtractFileName(EnvLib);
    Self.DllPath := ExtractFilePath(EnvLib);
  end;
  SetPythonHome(ExcludeTrailingBackslash(Self.DllPath));

  Self.LoadDll;
  RESULT := Self;
end;

function TWrithingPythonEngine.Configure(const PythonEnv, PythonVer: String): TWrithingPythonEngine;
begin
  Self.RegVersion := PythonVer;
  SetPythonHome(PythonEnv);
  var pythonFileDll  := 'python' + PythonVer.Replace('.', '') + '.dll';
  var pythonPath := IncludeTrailingBackslash(PythonEnv);

  if FileExists(pythonPath + pythonFileDll) then
  begin
    Self.DllName := pythonFileDll;
    Self.DllPath := pythonPath;
  end;

  Self.OnSysPathInit := OnInitSysPath;
  RESULT := Self;
end;

procedure TWrithingPythonEngine.CollectPyGarbage;
begin
  try
    ExecString('import gc');
    ExecString('gc.collect()');
    //ExecString('quit()')
  except on Ex: EPySystemExit do
  end;
end;

function TWrithingPythonEngine.Configure(const PythonFileDll: String): TWrithingPythonEngine;
begin
    Self.DllName := ExtractFileName(PythonFileDll);
    var pyVerCode := Self.DllName.Replace('python', '').Replace('.dll','');
    var majorVer := pyVerCode.Substring(0, 1);
    var minorVer := pyVerCode.Substring(1);
    Self.RegVersion := majorVer + '.' + minorVer;
    RESULT := Self;
end;

procedure TWrithingPythonEngine.OnInitSysPath(Sender: TObject;
  PathList: PPyObject);
const PACKAGES_SUFFIX = '\lib\site-packages';//\numpy\core
var newPath: String; //Ansi
begin
  //add to homepath syspath can resolve the issue?
  var PathPasList := TStringList.Create();
  PathPasList.Delimiter := ';';
  Self.PyListToStrings(PathList, PathPasList);
  //PathPasList.Insert(0, );
  Self.FIO.WriteLine(PathPasList.Text);
  PathList := Self.StringsToPyList(PathPasList);
  newPath := ';' + Self.FPythonHome + PACKAGES_SUFFIX + ';;' + PathPasList.DelimitedText;
  Self.PySys_SetPath(@newPath[1]);
  //Self.PySys_Set
  //Self.PySys_GetObject(
end;

procedure TWrithingPythonEngine.SoftCleanup;
var tempIO : TPythonInputOutput;
begin
  Self.CollectPyGarbage();

  tempIO := Self.IO;

  Self.Finalize();
  Self.FIOPythonModule := nil; //force to re-create python IO module

  Self.IO := tempIO;
  Self.RedirectIO := True;
end;

{ TPersistentPythonDelphiVar }

procedure TPersistentPythonDelphiVar.Init(value: Variant);
begin
  _StorageValue := value;
end;

procedure TPersistentPythonDelphiVar.RestoreValue;
begin
  if VarIsNull(_StorageValue) or VarIsEmpty(_StorageValue) then
    Exit;

  Self.Value := _StorageValue;
  _StorageValue := Null;
end;

procedure TPersistentPythonDelphiVar.StoreValue;
begin
  _StorageValue := Self.Value;
end;
